...
namespace ldso {
	...
	private:
		...
		Accumulator9 acc9[NUM_THREADS];
		Accumulator9 acc9SC[NUM_THREADS];
		Accumulator11 accE[NUM_THREADS];
		Accumulator11 accEAlpha[NUM_THREADS];
		IndexThreadReduce<Vec3> red;

		inline void setZero(int min = 0, int max = 1, Vec3 *stats = 0, int tid = 0) {
			acc9[tid].initialize();
			acc9SC[tid].initialize();
			accE[tid].initialize();
			accEAlpha[tid].initialize();
		}

		inline void finishAcc9(int min = 0, int max = 1, Vec3 *stats = 0, int tid = 0) {
			acc9[tid].finish();
		}

		inline void finishAcc9SC(int min = 0, int max = 1, Vec3 *stats = 0, int tid = 0) {
			acc9SC[tid].finish();
		}

		inline void finishAccE(int min = 0, int max = 1, Vec3 *stats = 0, int tid = 0) {
			accE[tid].finish();
		}

		inline void finishAccEAlpha(int min = 0, int max = 1, Vec3 *stats = 0, int tid = 0) {
			accEAlpha[tid].finish();
		}

		void calculateAllResiduals(Pnt *ptsl, Mat33f RKi, Vec3f t, Eigen::Vector2f r2new_aff, float fxl, float fyl, float cxl, float cyl, int wl, int hl, Eigen::Vector3f *colorRef, Eigen::Vector3f *colorNew, int min = 0, int max = 1, Vec3 *stats = 0, int tid = 0);

		void calculateAlphaEnergy(Pnt *ptsl, int min = 0, int max = 1, Vec3 *stats = 0, int tid = 0);

		void calculateSC(Pnt *ptsl, float alphaOpt, int min = 0, int max = 1, Vec3 *stats = 0, int tid = 0);
