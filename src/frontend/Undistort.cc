...
namespace ldso {
	...
	PhotometricUndistorter::PhotometricUndistorter(
		...
		LOG(INFO) << "Reading Photometric Calibration from file " << file.c_str();
		if (!f.good()) {
...

		{
			...
			if (GDepth < 256) {
				LOG(WARNING) << "PhotometricUndistorter: invalid format! got " << (int)Gvec.size() << " entries in first line, expected at least 256!";
				return;
			}
			...
			for (int i = 0; i < GDepth - 1; i++) {
				if (G[i + 1] <= G[i]) {
					LOG(WARNING) << "PhotometricUndistorter: G invalid! it has to be strictly increasing, but it isnt!";
					return;
				}
			}
		...
		LOG(INFO) << "Reading Vignette Image from " << vignetteImage.c_str();
		...
		if (vm16 != 0) {
			if (vm16->w != w || vm16->h != h) {
				LOG(INFO) << "PhotometricUndistorter: Invalid vignette image size! got " << vm16->w << " x " << vm16->h << " , expected " << w << " x " << h;
			...
			if (vm8->w != w || vm8->h != h) {
				LOG(INFO) << "PhotometricUndistorter: Invalid vignette image size! got " << vm8->w << " x " << vm8->h << ", expected " << w << " x " << h;
				if (vm16 != 0) delete vm16;
				if (vm8 != 0) delete vm8;
				return;
			}
			...
			LOG(WARNING) << "PhotometricUndistorter: Invalid vignette image";
			if (vm16 != 0) delete vm16;
			if (vm8 != 0) delete vm8;
			return;
		}
	...
	Undistort *Undistort::getUndistorterForFile(std::string configFilename, std::string gammaFilename,
		...
		LOG(INFO) << "Reading Calibration from file " << configFilename.c_str();

		std::ifstream f(configFilename.c_str());
		if (!f.good()) {
			f.close();
			LOG(WARNING) << configFilename.c_str() << " not found. Cannot operate without calibration, shutting down.";
			f.close();
			return 0;
		}

		LOG(INFO) << configFilename.c_str() << " found!";
		std::string l1;
		std::getline(f, l1);
		f.close();
		...
		// clean model selection implementation.
			...
			LOG(WARNING) << "could not read calib file! exit.";
			exit(1);
		}
	...
	ImageAndExposure *
		...
		if (image_raw->w != wOrg || image_raw->h != hOrg) {
			LOG(ERROR) << "Undistort::undistort: wrong image size (" << image_raw->w << " " << image_raw->h << " instead of " << w << " " << h << ")";
			exit(1);
		}
	...
	void Undistort::makeOptimalK_crop() {
		LOG(INFO) << "finding CROP optimal new model!";
		K.setIdentity();
		...
		LOG(INFO) << "initial range: x: " << minX << " - " << maxX << "; y: " << minY << " - " << maxY << "!";
		...
		while (oobLeft || oobRight || oobTop || oobBottom) {
			...
			LOG(INFO) << "iteration " << iteration << ": range: x: " << minX << " - " << maxX << "; y: " << minY << " - " << maxY << "!";
			if (iteration > 500) {
				LOG(ERROR) << "FAILED TO COMPUTE GOOD CAMERA MATRIX - SOMETHING IS SERIOUSLY WRONG. ABORTING";
				exit(1);
			}
		}
	...
	void Undistort::readFromFile(const char *configFileName, int nPars, std::string prefix) {
		...
		if (nPars == 5) // fov model
		{
			...
			if (std::sscanf(l1.c_str(), buf, &parsOrg[0], &parsOrg[1], &parsOrg[2], &parsOrg[3], &parsOrg[4]) == 5 &&
				...
				LOG(INFO) << "Input resolution: " << wOrg << " " << hOrg;
				LOG(INFO) << "In: " << parsOrg[0] << " " << parsOrg[1] << " " << parsOrg[2] << " " << parsOrg[3] << " " << parsOrg[4];
			}
			else {
				LOG(WARNING) << "Failed to read camera calibration (invalid format?)\nCalibration file: " << configFileName;
				infile.close();
				return;
			}
		}
		else if (nPars == 8) // KB, equi & radtan model
		{
			...
			if (std::sscanf(l1.c_str(), buf,
				...
				std::sscanf(l2.c_str(), "%d %d", &wOrg, &hOrg) == 2) {
				LOG(INFO) << "Input resolution: " << wOrg << " " << hOrg;
				LOG(INFO) << "In: " << prefix.c_str() << " " << parsOrg[0] << " " << parsOrg[1] << " " << parsOrg[2] << " " << parsOrg[3] << " " << parsOrg[4] << " " << parsOrg[5] << " " << parsOrg[6] << " " << parsOrg[7];
			}
			else {
				LOG(WARNING) << "Failed to read camera calibration (invalid format?)\nCalibration file:" << configFileName;
				infile.close();
			...
			LOG(WARNING) << "called with invalid number of parameters.... forgot to implement me?";
			infile.close();
			return;
		}
		...
		if (parsOrg[2] < 1 && parsOrg[3] < 1) {
			LOG(INFO) << "\n\nFound fx=" << parsOrg[0] << ", fy=" << parsOrg[1] << ", cx=" << parsOrg[2] << ", cy=" << parsOrg[3] << ".\n I'm assuming this is the \"relative\" calibration file format,"
				"and will rescale this by image width / height to fx=" << parsOrg[0] * wOrg << ", fy=" << parsOrg[1] * hOrg << ", cx=" << parsOrg[2] * wOrg - 0.5 << ", cy=" << parsOrg[3] * hOrg - 0.5 << ".";
		...
		if (l3 == "crop") {
			...
			LOG(INFO) << "Out: Rectify Crop";
		}
		else if (l3 == "full") {
			outputCalibration[0] = -2;
			LOG(INFO) << "Out: Rectify Full";
		}
		else if (l3 == "none") {
			outputCalibration[0] = -3;
			LOG(INFO) << "Out: No Rectification";
		}
			...
			LOG(INFO) << "Out: " << outputCalibration[0] << " " << outputCalibration[1] << " " << outputCalibration[2] << " " << outputCalibration[3] << " " << outputCalibration[4];
		}
		else {
		...
		if (std::sscanf(l4.c_str(), "%d %d", &w, &h) == 2) {
			...
			LOG(INFO) << "Output resolution: " << w << " " << h;
		}
		else {
		...
		if (outputCalibration[0] == -1)
			...
			if (w != wOrg || h != hOrg) {
				LOG(ERROR) << "ERROR: rectification mode none requires input and output dimenstions to match!";
				exit(1);
			}
			K.setIdentity();
			K(0, 0) = parsOrg[0];
			...
			if (outputCalibration[2] > 1 || outputCalibration[3] > 1) {
				LOG(WARNING) << "WARNING: given output calibration (" << outputCalibration[0] << " " << outputCalibration[1] << " " << outputCalibration[2] << " " << outputCalibration[3] << ") seems wrong. It needs to be relative to image width / height!";
			}
		...
		LOG(INFO) << "Rectified Kamera Matrix:";
		LOG(INFO) << K;
